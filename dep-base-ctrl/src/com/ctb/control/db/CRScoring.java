...
public interface CRScoring extends JdbcControl {
	...
	// score by item: all student list from test session and itemsetID
	 @JdbcControl.SQL(statement = "select roster.TEST_ROSTER_ID as testRosterId, roster.TEST_ADMIN_ID as testAdminId, tcsc.test_completion_status_desc testCompletionStatusDesc, stu.grade as grade, roster.STUDENT_ID as studentId, roster.CUSTOMER_ID as customerId, stu.first_name as firstName, stu.middle_name as middleName, stu.last_name as lastName,  stu.ext_pin1 as extPin1,  stu.user_name as userName   from student_item_set_status sis,  test_roster roster, student stu, test_completion_status_code tcsc, item it,item_response_cr irs where sis.item_set_id = {itemSetId} and roster.test_admin_id = {testAdminId} and roster.test_roster_id= sis.test_roster_id and stu.student_id = roster.student_id and tcsc.test_completion_status = roster.TEST_COMPLETION_STATUS and tcsc.test_completion_status in ('CO', 'IS','IC') and stu.activation_status='AC' and roster.activation_status='AC' and roster.validation_status in ( 'VA', 'PI') and sis.validation_status='VA' and irs.item_set_id = sis.item_set_id and irs.item_id = {itemId} and irs.item_id = it.item_id  and irs.test_roster_id = roster.test_roster_id  and  irs.constructed_response is not null and  (decode(it.answer_area,  null, decode(instr(constructed_response, 'CDATA'),  0,   0,  1),  1))=1",
             arrayMaxLength = 0, fetchSize=1000)
